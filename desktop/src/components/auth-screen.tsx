...
export function AuthScreen() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      if (isRegister) {
        await register({ email: form.email, username: form.username, displayName: form.displayName || form.username, password: form.password });
    ...
    <div className="grid min-h-screen place-items-center p-6">
      <motion.form onSubmit={submit} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card-glass w-full max-w-md rounded-3xl p-6">
        ...
        <div className="space-y-3">
          {isRegister ? (
            <>
              <input className="w-full rounded-xl border border-white/10 bg-[#131a2d] px-3 py-2" placeholder="Display name (optional)" onChange={(e) => setForm({ ...form, displayName: e.target.value })} />
        ...
        <p className="mt-4 text-xs text-slate-400">Demo users: demo/demo123, maya/maya123, leo/leo123</p>
      </motion.form>
    </div>
  );
}
